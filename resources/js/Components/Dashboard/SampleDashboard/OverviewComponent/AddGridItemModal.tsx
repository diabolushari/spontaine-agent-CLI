...
function AddGridItemModal({
  ...
  const renderFilterRow = (filter: Filter) => {
    ...
    const selectedDimensions = filters.filter((f) => f.id !== filter.id).map((f) => f.dimension)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {error && (
          <div className='mt-4 rounded-md border border-red-300 bg-red-100 p-3 text-red-800'>
            {error}
          </div>
        )}
